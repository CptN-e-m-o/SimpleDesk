...
import type { QueueHealthResult, QueueHealthStatus } from '../queueTypes'

type Props = {
    configurationId: number
    onResult?: (result: QueueHealthResult) => void
    className?: string
}
...
export default function QueueTestButton({
                                            configurationId,
                                            onResult,
                                            className = '',
                                        }: Props) {
    ...
    const test = async () => {
        ...
        try {
            const response = await fetch(
                route('admin.system.queues.test', configurationId),
                {
                    method: 'POST',
            ...
            if (!isQueueHealthResult(payload)) {
                throw new Error(
                    'The Queue health check returned an unexpected response.',
                )
            }

            /*
             * degraded / unhealthy / unavailable are valid health-check
             * results. They are not HTTP request failures.
             */
            onResult?.(payload)
        } catch (reason) {
            ...
            setError(
                reason instanceof Error
                    ? reason.message
                    : 'The Queue test could not be completed.',
            )
        } finally {
...
function responseErrorMessage(status: number, payload: unknown): string {
    ...
    if (status === 403) {
        return 'You do not have permission to test this Queue configuration.'
    }

    if (status === 404) {
        return 'This Queue configuration is no longer available.'
    }

    return 'The Queue test could not be completed.'
}
...
function isQueueHealthResult(payload: unknown): payload is QueueHealthResult {
    if (!isRecord(payload)) {
        return false
    }

    if (!isQueueHealthStatus(payload.status)) {
        return false
    }
...
function isQueueHealthStatus(value: unknown): value is QueueHealthStatus {
    return typeof value === 'string' && healthStatuses.has(value)
}
