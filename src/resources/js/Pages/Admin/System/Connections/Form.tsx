...
export default function Form({ definitions, connection }: Props) {
    ...
    const initialSource =
        connection?.source ?? getDefaultSource(initialDefinition)
    ...
    const form = useForm<FormData>({
        ...
        configuration:
            ...
            ?? createDefaultConfiguration(initialDefinition),
        credentials: {
            password: '',
            app_key: '',
            app_secret: '',
    ...
    const passwordConfigured = Boolean(
        connection?.credential_flags?.password_configured,
    )

    const removePassword =
        form.data.remove_credentials.includes('password')

    const availableSources =
        definition?.sources ?? []

    const deploymentConnections =
        definition?.options.deployment_connections ?? []
    ...
    const submit = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (connection) {
            form.put(
                route(
                    'admin.system.connections.update',
                    connection.id,
                ),
            )
        ...
        form.post(
            route(
                'admin.system.connections.store',
            ),
        )
    }
    ...
    const changeType = (type: string) => {
        ...
        const source =
            getDefaultSource(nextDefinition)
    ...
    const setPassword = (password: string) => {
        form.setData((data) => ({
            ...
            remove_credentials:
                password !== ''
                    ? data.remove_credentials.filter(
                        (item) =>
                            item !== 'password',
                    )
                    : data.remove_credentials,
    ...
    return (
        ...
        >
            ...
            {form.data.source === 'deployment' ? (
                ...
                />
            ) : isSimpleSearchProviderType(
                form.data.type,
            ) ? (
                <ManagedSearchProviderConfiguration
                    type={form.data.type}
                    configuration={
                        form.data.configuration
                    }
                ...
                />
            ) : form.data.type === 'typesense' ? (
                <ManagedTypesenseConfiguration
                    configuration={
                        form.data.configuration
                    }
                    apiKey={
                ...
                <section className="rounded-[28px] border border-amber-200 bg-amber-50 p-5">
                    <p className="font-semibold text-amber-900">
                        Managed configuration unavailable
                    </p>
                    ...
                    >
                        <div className="relative">
                            <input
                                ...
                                )} pr-11 disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400`}
                            />
...
function createDefaultConfiguration(
    ...
    if (definition?.type === 'meilisearch') {
        return {
            host: 'http://127.0.0.1:7700',
        }
    }

    if (definition?.type === 'typesense') {
        return {
            nodes: [
                {
                    host: '127.0.0.1',
...
function isPusherProtocolType(
    type: string,
): type is 'reverb' | 'pusher' {
    return (
        type === 'reverb'
        || type === 'pusher'
    )
...
function isSimpleSearchProviderType(
    type: string,
): type is 'meilisearch' | 'algolia' {
    return (
        type === 'meilisearch'
        || type === 'algolia'
    )
...
function connectionNamePlaceholder(
    type: string,
): string {
    if (type === 'reverb') {
    ...
    if (type === 'meilisearch') {
        return 'Production Meilisearch'
    }

    if (type === 'typesense') {
        return 'Production Typesense'
    }
