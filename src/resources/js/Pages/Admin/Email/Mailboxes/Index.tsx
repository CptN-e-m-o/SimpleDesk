...
import DeleteMailboxModal, {
    MailboxAction,
} from './DeleteMailboxModal'
import {
    Mailbox,
...
function calculateSummary(
    ...
    const activeMailboxes = mailboxes.filter(
        (mailbox) => !mailbox.is_deleted,
    )

    return {
        total: mailboxes.length,

        active: activeMailboxes.filter(
            (mailbox) => mailbox.is_active,
        ).length,

        configured: activeMailboxes.filter(
            isMailboxConfigured,
        ).length,

        healthy: activeMailboxes.filter(
            (mailbox) =>
                getMailboxOverallStatus(mailbox) ===
                'healthy',
        ).length,

        needs_attention: activeMailboxes.filter(
            (mailbox) =>
                mailbox.is_active &&
                getMailboxOverallStatus(mailbox) !==
                'healthy',
            ...
            return (
                <>
                    <button
                        ...
                        className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-2xl border border-emerald-200 bg-emerald-50 text-emerald-700 transition hover:bg-emerald-100"
                    >
                    ...
                    <button
                        type="button"
                        title="Delete mailbox permanently"
                        onClick={() =>
                            setPendingAction({
                                mailbox,
                                action: 'force-delete',
                        ...
                        className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-2xl border border-rose-200 bg-rose-50 text-rose-700 transition hover:bg-rose-100"
                    >
        ...
        return (
            <>
                ...
                <button
                    ...
                    className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-2xl border border-gray-200 bg-white text-rose-600 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700"
                >
                    <Trash2 className="h-4 w-4" />
                </button>
            </>
            ...
            return (
                <>
                    <button
                        ...
                        className="inline-flex h-10 flex-1 cursor-pointer items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 text-sm font-medium text-emerald-700 transition hover:bg-emerald-100"
                    >
                        <ArchiveRestore className="h-4 w-4" />
                        Restore
                    </button>
                ...
                <section className="overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-sm">
                    <div className="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white px-6 py-5">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                                ...
                                <Link
                                    href={route(
                                        'admin.email.settings.mailboxes.create',
                                    )}
                    ...
                    <div className="border-b border-gray-100 bg-gray-50/70 px-6 py-4">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                            ...
                            <SummaryCard
                                label="Configured"
                                value={effectiveSummary.configured}
                                icon={
                            ...
                            <SummaryCard
                                label="Healthy"
                                value={effectiveSummary.healthy}
                                icon={
                                    <CircleCheck className="h-5 w-5" />
                                }
                                                ...
                                                >
                                                    <td className="px-5 py-5">
                                                        <div className="min-w-56">
                                                            <div className="flex flex-wrap items-center gap-2">
                                                                ...
                                                                {mailbox.is_deleted ? (
                                                                    <span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-gray-600 ring-1 ring-inset ring-gray-200">
                                                                            Deleted
                                                                        </span>
                                                                ) : null}
                                                            </div>
                                                    ...
                                                    <td className="px-5 py-5">
                                                        <div className="flex items-center justify-end gap-2">
                                                            <Link
                                                                href={route(
                                                                    'admin.email.settings.mailboxes.show',
                                                                    mailbox.id,
                                                                )}
                                                                title="View mailbox"
                                                                className="inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-2xl border border-gray-200 bg-white text-gray-600 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700"
                            ...
                            <div className="flex flex-col items-center justify-center rounded-[24px] border border-dashed border-gray-300 bg-gray-50 px-6 py-14 text-center">
                                ...
                                {search ? (
                                    ...
                                    <Link
                                        ...
                                        className="mt-6 inline-flex h-11 cursor-pointer items-center justify-center gap-2 rounded-2xl bg-sky-600 px-4 text-sm font-medium text-white transition hover:bg-sky-700"
                                    >
                                        <Plus className="h-4 w-4" />
                                        Create Mailbox
                                    </Link>
