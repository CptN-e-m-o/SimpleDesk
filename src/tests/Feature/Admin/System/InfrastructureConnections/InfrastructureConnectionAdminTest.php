...
class InfrastructureConnectionAdminTest extends TestCase
{
    ...
    public function test_catalog_creates_updates_archives_and_restores_connection(): void
    {
        ...
        $connection = $service->create(['name' => 'Redis', 'type' => 'redis', 'source' => 'managed', 'configuration' => ['host' => 'redis'], 'credentials' => ['password' => 'first-secret']], $actor);
        $service->update($connection, ['name' => 'Redis updated', 'source' => 'managed', 'configuration' => ['host' => 'redis-new'], 'credentials' => ['password' => '']], $actor);
        $this->assertSame('first-secret', $connection->refresh()->secrets()['password']);
        $service->update($connection, ['name' => 'Redis updated', 'source' => 'managed', 'configuration' => [], 'remove_credentials' => ['password']], $actor);
        $this->assertArrayNotHasKey('password', $connection->refresh()->secrets());
        $service->archive($connection, $actor);
        $this->assertSoftDeleted($connection);
        $this->assertFalse($service->restore($connection->id, $actor)->trashed());
    }
}
