    ...
    public function run(): void
    {
        DB::transaction(function () {
            ...
            User::whereHas('roles', fn ($q) => $q->where('type', 'agent'))->eachById(function (User $agent) use ($default) {
                if (! AgentStatusPeriod::forAgent($agent)->global()->open()->exists()) AgentStatusPeriod::create(['user_id' => $agent->id, 'agent_status_id' => $default->id, 'scope' => 'global', 'started_at' => now(), 'source' => 'system']);
            });
        });
    }
}
