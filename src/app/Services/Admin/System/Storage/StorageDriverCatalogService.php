...
{
    public function __construct(private readonly StorageDriverRegistry $registry, private readonly SystemAuditLogger $audit) {}
    ...
    public function create(array $data, User $actor): StorageDriverConfiguration
    {
        return DB::transaction(function () use ($data, $actor) {
            ...
            if (! $type) {
                throw ValidationException::withMessages(['driver' => 'The selected Storage driver is unavailable.']);
            }
            $normalized = $this->registry->adapter($type)->validateAndNormalize($data['configuration'] ?? [], $data['infrastructure_connection_id'] ?? null);
            $model = StorageDriverConfiguration::query()->create(['name' => $data['name'], 'driver' => $type, ...$normalized, 'is_enabled' => $data['is_enabled'] ?? true, 'created_by' => $actor->id, 'updated_by' => $actor->id]);
    ...
    public function update(StorageDriverConfiguration $model, array $data, User $actor): StorageDriverConfiguration
    {
        return DB::transaction(function () use ($model, $data, $actor) {
            $model = StorageDriverConfiguration::withTrashed()->lockForUpdate()->findOrFail($model->id);
            $this->guardInactive($model, $this->lockSettings());
            if (isset($data['driver']) && $data['driver'] !== $model->getRawOriginal('driver')) {
                throw ValidationException::withMessages(['driver' => 'The Storage driver cannot be changed.']);
            }
    ...
    public function setEnabled(StorageDriverConfiguration $model, bool $enabled, User $actor): StorageDriverConfiguration
    {
        return $this->mutate($model, $actor, $enabled ? 'enable' : 'disable', fn (StorageDriverConfiguration $locked) => $locked->update(['is_enabled' => $enabled, 'updated_by' => $actor->id]));
    }

    public function archive(StorageDriverConfiguration $model, User $actor): void
    {
        $this->mutate($model, $actor, 'archive', fn (StorageDriverConfiguration $locked) => $locked->delete());
    }

    public function restore(int $id, User $actor): StorageDriverConfiguration
    {
        return DB::transaction(function () use ($id, $actor) {
            $model = StorageDriverConfiguration::onlyTrashed()->lockForUpdate()->findOrFail($id);
    ...
    public function forceDelete(int $id, User $actor): void
    {
        DB::transaction(function () use ($id, $actor) {
            ...
            $before = $this->safe($model);
            $model->forceDelete();
            $this->audit->log('storage_driver_configurations', 'force_delete', StorageDriverConfiguration::class, $id, $before, null, actor: $actor);
        });
    }
    ...
    private function mutate(StorageDriverConfiguration $model, User $actor, string $action, callable $callback): StorageDriverConfiguration
    {
        return DB::transaction(function () use ($model, $actor, $action, $callback) {
            $locked = StorageDriverConfiguration::withTrashed()->lockForUpdate()->findOrFail($model->id);
    ...
    {
        return StorageDriverSettings::query()->whereKey(1)->lockForUpdate()->first();
    }

    private function guardInactive(StorageDriverConfiguration $model, ?StorageDriverSettings $settings): void
    {
        if ($settings?->getRawOriginal('mode') === StorageConfigurationMode::Managed->value && $settings->active_configuration_id === $model->id) {
            throw ValidationException::withMessages(['configuration' => 'The active managed Storage configuration cannot be mutated.']);
        }
    }

    private function log(string $action, StorageDriverConfiguration $model, ?array $before, ?array $after, User $actor): void
    {
        $this->audit->log('storage_driver_configurations', $action, StorageDriverConfiguration::class, $model->id, $before, $after, actor: $actor);
    }
}
