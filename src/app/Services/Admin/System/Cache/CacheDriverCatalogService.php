...
class CacheDriverCatalogService
{
    ...
    public function create(array $data, User $actor): CacheDriverConfiguration
    {
        return DB::transaction(function () use ($data, $actor) {
            $type = CacheDriverType::tryFrom((string) ($data['driver'] ?? ''));
            if (! $type) {
                throw ValidationException::withMessages(['driver' => 'Unknown cache driver.']);
            } $this->registry->adapter($type);
            $infra = $this->resolveInfrastructure($type, $data['infrastructure_connection_id'] ?? null);
            $normalized = $this->normalize($type, $data['configuration'] ?? []);
            $model = CacheDriverConfiguration::query()->create(['name' => $data['name'], 'driver' => $type, 'infrastructure_connection_id' => $infra?->id, 'configuration' => $normalized, 'is_enabled' => $data['is_enabled'] ?? true, 'created_by' => $actor->id, 'updated_by' => $actor->id]);
    ...
    public function update(CacheDriverConfiguration $model, array $data, User $actor): CacheDriverConfiguration
    {
        return DB::transaction(function () use ($model, $data, $actor) {
            ...
            $model = CacheDriverConfiguration::query()->lockForUpdate()->findOrFail($model->id);
            $this->guardInactive($model, $settings);
            if (isset($data['driver']) && $data['driver'] !== $model->driver->value) {
                throw ValidationException::withMessages(['driver' => 'Cache driver cannot be changed after creation.']);
            } $before = $this->safe($model);
            $infra = $this->resolveInfrastructure($model->driver, array_key_exists('infrastructure_connection_id', $data) ? $data['infrastructure_connection_id'] : $model->infrastructure_connection_id);
            $model->update(['name' => $data['name'], 'infrastructure_connection_id' => $infra?->id, 'configuration' => $this->normalize($model->driver, $data['configuration'] ?? []), 'is_enabled' => $data['is_enabled'] ?? $model->is_enabled, 'updated_by' => $actor->id]);
    ...
    public function archive(CacheDriverConfiguration $model, User $actor): void
    {
    ...
    public function restore(int $id, User $actor): CacheDriverConfiguration
    {
        return DB::transaction(function () use ($id, $actor) {
            $model = CacheDriverConfiguration::onlyTrashed()->lockForUpdate()->findOrFail($id);
            $model->restore();
            $model->update(['is_enabled' => false, 'updated_by' => $actor->id]);
    ...
    public function forceDelete(int $id, User $actor): void
    {
        DB::transaction(function () use ($id, $actor) {
            ...
            $subject = $model->id;
            $model->forceDelete();
            $this->audit->log('cache_driver_configurations', 'force_delete', CacheDriverConfiguration::class, $subject, $before, null, actor: $actor);
        });
    }

    public function safe(CacheDriverConfiguration $model): array
    {
        $model->loadMissing(['latestHealthCheck', 'infrastructureConnection']);
    ...
    private function normalize(CacheDriverType $type, array $configuration): array
    {
        unset($configuration['infrastructure_connection_id']);
        try {
    ...
    private function resolveInfrastructure(CacheDriverType $type, mixed $id): ?InfrastructureConnection
    {
        if ($type !== CacheDriverType::Redis) {
            if ($id !== null && $id !== '') {
                throw ValidationException::withMessages(['infrastructure_connection_id' => 'Only Redis cache configurations may reference infrastructure.']);
            }
...
return null;
        ...
        if ($id === false) {
            throw ValidationException::withMessages(['infrastructure_connection_id' => 'A Redis infrastructure connection is required.']);
        } $connection = InfrastructureConnection::withTrashed()->find($id);
        if (! $connection) {
            throw ValidationException::withMessages(['infrastructure_connection_id' => 'The selected Redis infrastructure connection does not exist.']);
        } if ($connection->trashed()) {
            throw ValidationException::withMessages(['infrastructure_connection_id' => 'The selected Redis infrastructure connection is archived.']);
        } if ($connection->type !== InfrastructureConnectionType::Redis) {
            throw ValidationException::withMessages(['infrastructure_connection_id' => 'The selected infrastructure connection is not Redis.']);
        } if (! $connection->is_enabled) {
            throw ValidationException::withMessages(['infrastructure_connection_id' => 'The selected Redis infrastructure connection is disabled.']);
        }
...
return $connection;
    ...
    private function guardInactive(CacheDriverConfiguration $model, ?CacheDriverSettings $settings): void
    {
        if ($settings?->mode === CacheConfigurationMode::Managed && $settings->active_configuration_id === $model->id) {
            throw new ActiveCacheDriverConfigurationMutationException('The active managed cache configuration cannot be mutated.');
        }
    }

    private function isActive(CacheDriverConfiguration $model): bool
    {
        return CacheDriverSettings::query()->whereKey(1)->where('mode', CacheConfigurationMode::Managed->value)->where('active_configuration_id', $model->id)->exists();
    }

    private function log(string $action, CacheDriverConfiguration $model, ?array $before, ?array $after, User $actor): void
    {
        $this->audit->log('cache_driver_configurations', $action, CacheDriverConfiguration::class, $model->id, $before, $after, actor: $actor);
    }
}
