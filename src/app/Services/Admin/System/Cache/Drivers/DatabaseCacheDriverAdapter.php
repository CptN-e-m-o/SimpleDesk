...
class DatabaseCacheDriverAdapter implements CacheDriverAdapter
{
    ...
    public function type(): CacheDriverType { return CacheDriverType::Database; }
    public function definition(): CacheDriverDefinitionData { return new CacheDriverDefinitionData($this->type(), 'Database', 'Use Laravel database cache and lock tables.', false, null, true, options: ['database_connections' => $this->allowedConnections()]); }
    ...
    {
        $runtime = $this->runtimeConfiguration($configuration); $connection = $runtime->store['connection'];
        if (! Schema::connection($connection)->hasTable('cache') || ! Schema::connection($connection)->hasTable('cache_locks')) return new CacheHealthResultData(\App\Enums\Admin\System\CacheHealthStatus::Unhealthy, 0, 'The configured database cache and lock tables are required.', ['database_connection' => $connection, 'cache_table' => 'cache', 'lock_table' => 'cache_locks']);
    ...
    {
        $available = array_keys((array) config('database.connections', [])); $configured = array_values(array_unique(array_filter(array_map(fn ($v) => trim((string) $v), (array) config('simpledesk-cache.database.allowed_connections', [])))));
        if ($configured !== []) return array_values(array_intersect($configured, $available));
        $default = trim((string) config('database.default', '')); return $default !== '' && in_array($default, $available, true) ? [$default] : [];
    }
}
