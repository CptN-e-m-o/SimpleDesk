...
class AgentStatusResolver
{
    ...
    public function currentStatus(User $agent, ?AgentWorkChannel $channel = null, ?CarbonInterface $at = null): ResolvedAgentStatus
    {
        ...
        $routing = $base->routing_eligibility;
        $display = $base;
        if ($channelPeriod) {
            if ($channelPeriod->status->availability->weight() > $availability->weight()) {
                $availability = $channelPeriod->status->availability;
                $display = $channelPeriod->status;
            }
            if ($channelPeriod->status->routing_eligibility->weight() > $routing->weight()) {
                $routing = $channelPeriod->status->routing_eligibility;
                $display = $channelPeriod->status;
            }
        }
    ...
    public function availabilityFor(User $agent, ?AgentWorkChannel $channel = null, ?CarbonInterface $at = null): AgentStatusAvailability
    {
        return $this->currentStatus($agent, $channel, $at)->availability;
    }

    public function routingEligibilityFor(User $agent, ?AgentWorkChannel $channel = null, ?CarbonInterface $at = null): AgentRoutingEligibility
    {
        return $this->currentStatus($agent, $channel, $at)->routingEligibility;
    }

    public function canReceiveNewWork(User $agent, ?AgentWorkChannel $channel = null, ?CarbonInterface $at = null): bool
    {
        $resolved = $this->currentStatus($agent, $channel, $at);

        return $resolved->availability === AgentStatusAvailability::Available && $resolved->routingEligibility === AgentRoutingEligibility::Eligible;
    }

    public function history(User $agent, array $filters = [], int $perPage = 25)
    {
        return AgentStatusPeriod::with(['status', 'setBy', 'endedBy'])->forAgent($agent)->when($filters['status_id'] ?? null, fn ($q, $v) => $q->where('agent_status_id', $v))->when($filters['source'] ?? null, fn ($q, $v) => $q->where('source', $v))->when($filters['scope'] ?? null, fn ($q, $v) => $q->where('scope', $v))->when($filters['channel'] ?? null, fn ($q, $v) => $q->where('channel', $v))->when($filters['from'] ?? null, fn ($q, $v) => $q->where('started_at', '>=', $v))->when($filters['to'] ?? null, fn ($q, $v) => $q->where('started_at', '<=', $v))->history()->paginate($perPage)->withQueryString();
    }
}
