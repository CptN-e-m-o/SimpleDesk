...
class AgentStatusExpirationService
{
    public function expireDueStatuses(): array
    {
        ...
        AgentStatusPeriod::expired()->orderBy('id')->chunkById(100, function ($periods) use (&$result) {
            foreach ($periods as $period) { $result['found']++; try { $this->expirePeriod($period) ? $result['expired']++ : $result['skipped']++; } catch (Throwable $e) { report($e); $result['errors']++; } }
        });
        return $result;
    }
    public function expirePeriod(AgentStatusPeriod $period): bool
    {
        return DB::transaction(function () use ($period) {
            ...
            if (! $period || $period->ended_at || ! $period->expires_at || $period->expires_at->isFuture()) return false;
            ...
            if (! $open) AgentStatusPeriod::create(['user_id' => $period->user_id, 'agent_status_id' => $revert->id, 'scope' => $period->scope, 'channel' => $period->channel, 'started_at' => now(), 'source' => AgentStatusSource::System]);
            return true;
        });
    }
}
