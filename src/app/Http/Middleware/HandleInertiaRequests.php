    ...
    public function share(Request $request): array
    {
        ...
        return [
            ...
            'agentStatus' => $agent ? fn () => [
                'current' => (function ($resolved) { return ['id' => $resolved->status->id, 'name' => $resolved->status->name, 'icon' => $resolved->status->icon, 'color' => $resolved->status->color, 'availability' => $resolved->availability->value, 'expires_at' => $resolved->globalPeriod?->expires_at?->toIso8601String()]; })(app(AgentStatusResolver::class)->currentStatus($agent)),
